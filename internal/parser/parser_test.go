...
func TestParse_AggregatesHourlyByHost(t *testing.T) {
	path := writeTemp(t, csvHeader+"\n"+
		`"2026-09-14T01:01:00Z","Rule A","high","BUH3","Sec","5376","1","User: Work","x","id1"`+"\n"+
		`"2026-09-14T01:02:00Z","Rule A","high","BUH3","Sec","5376","2","User: Work","x","id1"`+"\n"+
		`"2026-09-14T01:03:00Z","Rule B","med","BUH3","Sec","4624","3","Type: 3","y","id2"`+"\n"+
		`"2026-09-15T05:00:00Z","Rule C","low","BUH","Sys","7045","4","Svc: X","z","id3"`+"\n")
	...
	if len(data.Hosts) != 2 {
		t.Fatalf("хостов: %d, ожидала 2 (BUH3, BUH)", len(data.Hosts))
	}
	buh3 := data.Hourly["BUH3"]
	if len(buh3) != 1 {
		t.Fatalf("часовых точек BUH3: %d, ожидала 1 (всё в час 01)", len(buh3))
	}
	if buh3[0].T != "2026-09-14T01" {
		t.Errorf("ключ часа = %q, ожидала 2026-09-14T01", buh3[0].T)
	}
	if buh3[0].N != 3 {
		t.Errorf("N = %d, ожидала 3 события в час", buh3[0].N)
	}
	if got := data.Hourly["BUH"][0].N; got != 1 {
		t.Errorf("N BUH = %d, ожидала 1", got)
	}
}
...
func TestParse_TopRulesSortedAndLimited(t *testing.T) {
	lines := csvHeader + "\n"
	for i := 0; i < 10; i++ {
		lines += `"2026-09-14T01:00:00Z","Rule A","info","H","Sec","1","` + itoa(i) + `","d","e","r"` + "\n"
	}
	for i := 0; i < 3; i++ {
		lines += `"2026-09-14T01:00:00Z","Rule B","info","H","Sec","1","` + itoa(100+i) + `","d","e","r"` + "\n"
	}
	for i := 0; i < 9; i++ {
		lines += `"2026-09-14T01:00:00Z","Rule C","info","H","Sec","1","` + itoa(200+i) + `","d","e","r"` + "\n"
	}
...
func TestParse_MinLevelFilter(t *testing.T) {
	path := writeTemp(t, csvHeader+"\n"+
		`"2026-09-14T01:00:00Z","R info","info","H","Sec","1","1","d","e","r"`+"\n"+
		`"2026-09-14T02:00:00Z","R low","low","H","Sec","1","2","d","e","r"`+"\n"+
		`"2026-09-14T03:00:00Z","R high","high","H","Sec","1","3","d","e","r"`+"\n")
	...
	if pts[0].T != "2026-09-14T02" || pts[1].T != "2026-09-14T03" {
...
func TestParse_SamplesCappedAtTwelve(t *testing.T) {
	lines := csvHeader + "\n"
	for i := 0; i < 20; i++ {
		lines += `"2026-09-14T01:00:00Z","R","info","H","Sec","1","` + itoa(i) + `","d","e","r"` + "\n"
	}
	data, err := Parse(writeTemp(t, lines), Options{MinLevel: domain.LevelInfo})
	if err != nil {
		t.Fatal(err)
...
func TestParse_SkipsMalformedRows(t *testing.T) {
	path := writeTemp(t, csvHeader+"\n"+
		...
		`"short","row"`+"\n"+
		`"2026-09-14T02:00:00Z","R","unknown-level","H","Sec","1","2","d","e","r"`+"\n")
...
func TestParse_HostsSorted(t *testing.T) {
	path := writeTemp(t, csvHeader+"\n"+
		`"2026-09-14T01:00:00Z","R","info","Zeta","Sec","1","1","d","e","r"`+"\n"+
		`"2026-09-14T01:00:00Z","R","info","Alpha","Sec","1","2","d","e","r"`+"\n"+
		`"2026-09-14T01:00:00Z","R","info","Mid","Sec","1","3","d","e","r"`+"\n")
